...
import { ServiceFormData, ServiceType, PricingModel, InsuranceProvider } from '../onboarding/step-2/types';
import { Plus, X, Check, Layers, Package, ShieldCheck } from 'lucide-react';

import { useRemoveChildService } from '@/hooks/use-provider-services-hook';
...
export const ProviderServiceFormSection: React.FC<FormSectionProps> = ({ 
  formData, 
  setFormData, 
  onSubmit, 
  onCancel, 
  ...
  const [removingServiceIndex, setRemovingServiceIndex] = useState<number | null>(null);
  const removeChildService = useRemoveChildService();
  ...
  const removePackageService = async (index: number) => {
    const serviceName = formData.includedServices[index];
    
    // If editing an existing package, remove from database
    if (isEditing && serviceId) {
      setRemovingServiceIndex(index);
      try {
        await removeChildService.mutateAsync({
          packageId: serviceId,
        ...
        setRemovingServiceIndex(null);
      }
    } else {
      // If creating new package, just remove from local state
      setFormData(prev => ({
        ...prev,
        includedServices: prev.includedServices.filter((_, i) => i !== index)
      }));
  ...
  return (
    <div className="bg-white dark:bg-slate-800 flex flex-col">
      <div className="p-6 md:p-8 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50">
        ...
        <p className="text-slate-500 dark:text-slate-400 mt-1">Define the details, pricing, and coverage options.</p>
      </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-lg border border-slate-200 dark:border-slate-600 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500 resize-none"
            />
          </div>
        </div>
          ...
          <div className="space-y-3 bg-purple-50 p-5 rounded-xl border border-purple-100">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2.5 rounded-lg border border-purple-200 dark:border-purple-700 dark:bg-slate-700 dark:text-white focus:ring-2 focus:ring-purple-500 outline-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
              />
              <button
      ...
      <div className="p-6 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex gap-3">
        <button 
          ...
          className="flex-1 bg-white dark:bg-slate-800 border-2 border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 font-semibold py-3 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
        >
          Cancel
        </button>
        <button 
          ...
          disabled={isSubmitting}
          className="flex-1 bg-indigo-600 text-white font-semibold py-3 rounded-lg hover:bg-indigo-700 transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isSubmitting ? (
            <>
